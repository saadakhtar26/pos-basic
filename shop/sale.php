			  ...
			  <form class="form-horizontal00 billingForm" action="ajax_sale.php" method="POST" name="billingForm" id="dd" autocomplete="off">
				...
				<table id="app">
					...
					<tr id="1">
						...
						<td>
							<input type="number" class="form-control" oninput="calculate_price(this.value,1)" step="1" id="quantity_1" min="1" name="quantity[]" />
						</td>
						<td>
							<input value="0" type="number" class="form-control" oninput="calculate_price_disc(1)" id="discount_1" name="discount[]" min="0" max="100" step="5" />
						</td>
						<td>
					...
					<tfoot id="foot" style="margin-top:20px;">
						<tr>
							...
							<!-- Hidden Pending Inputs -->
							<input style="visibility: hidden;" name="received" type="number" />
							<input style="visibility: hidden;" name="pending" type="number" />
							<input style="visibility: hidden;" name="due_date" type="text" value="" />
							<!-- Hidden Pending Inputs -->
							<!--Pending Switch End-->
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.type == 'Success'){
				...
				let received = document.querySelector('input[name=received]');
				
				getTotal.value = Math.round(parseFloat(aac));
				received.value = getTotal.value;
...
function calculate_price(q,n){
	...
	let discount = document.getElementById('discount_'+n);
	var total_before = rate * q;
	var disc_per_unit = ((discount.value) / 100) * rate;
	var total_disc = disc_per_unit * q;
	var total_after = total_before.toFixed(2) - total_disc.toFixed(2);
...
function calculate_price_disc(n){
	...
	//var total = document.getElementById('getTotal').value;
	var gt = document.getElementById('sale_price_'+n).value = total_after.toFixed(2);
	...
	$.ajax({
		...
		success: function(dataSuccess){
			...
			if(dataSuccess != "ERROR"){
				newWin= window.open("");
				newWin.location.replace('../view_invoice.php?id='+dataSuccess);
				newWin.print();
				//newWin.close();
