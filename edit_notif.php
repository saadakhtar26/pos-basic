...
<body>
                            ...
                            <div class="card col-md-12">
                                ...
                                <div class="card-body card-block">
                                    <form action="edit_notif.php" method="post">
                                        <!--Hidden ID input-->
                                        <?php
                                        $sql3 = "SELECT `chain_id` FROM `invoice` WHERE `id`=" . $id;
                                        ...
                                        if($target_chain_id!=0){
                                            ...
                                            $sql4 = "SELECT SUM(received) FROM `invoice` WHERE `id`=" . $id;
                                            $result4 = mysqli_query($conn,$sql4);
                                            if(mysqli_num_rows($result4)>0){
                                                $row4 = mysqli_fetch_assoc($result4);
                                                $total_received = $row4["SUM(received)"];
    ...
    <?php }
        ...
        if($_POST["pending"]==0){
            $pending_status = 0;
            $sql5 = "UPDATE `invoice` SET `pending_status`=0 WHERE `chain_id`=" . $chain_id . " OR `id`=" . $chain_id;
    ...
    if (($_SERVER["REQUEST_METHOD"]=="POST") && ($staus==true)) {
        echo "<script>location.href='pending/index.php?page=3';</script>";
    }
